...
"""Based on the dictionary of locations returned by photos.geotag_dir(),
  ...
  Returns:
     new directory name in format: 
        Seychelles - Praslin (Ferdinand Nature Reserve, CatCocos), La Digue
"""
...
def _generate_name(locations, directory):
    ...
    for area, places_orig in areas_orig.items():
        ...
        if len(places) > 0:
            ...
            areas.append(area)
    new_name = directory + " " + country + " - " + ', '.join(
        areas) if country else directory + " " + ', '.join(areas)
    return new_name
...
"""Rename the folders with the name computed by the generate_name function
   ...
   Args:
        ...
        directory_original_name: original absolut path of the directory
...
def rename(directory, directory_original_name, location, dry_run=False):
    ...
    if directory_new_name is None:
        return None
    log.info(directory + ' -> ' + directory_new_name)
    if not dry_run:
        try:
